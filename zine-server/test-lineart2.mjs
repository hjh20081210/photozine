...
async function extractLineArt(imgBuf, opts = {}) {
  const { threshold = 0.45, blur = 0.5, dilateRadius = 1 } = opts;
  ...
  for (let i = 0; i < w * h; i++) {
    const o = i * 4;
    if (edges[i] > thresh) { out[o] = 0; out[o + 1] = 0; out[o + 2] = 0; out[o + 3] = 255; blackCount++; }
    else { out[o + 3] = 0; }
  }
...
const configs = [
  { name: 'crisp', threshold: 0.60, blur: 0.3, dilateRadius: 1 },
  { name: 'fine', threshold: 0.70, blur: 0.3, dilateRadius: 1 },
  { name: 'bold', threshold: 0.55, blur: 0.5, dilateRadius: 1 },
];
