...
function buildFrontArtPrompt(body) {
  ...
  return `你是一位资深的水彩/油画艺术大师。现在请你对用户上传的照片进行写生，将其中的景物（静物）创作成一幅精美的${stylePrompt}画作。创作要求：1）忠实于原照片的构图、视角、主体位置、大小比例、远近层次，确保观者能一眼认出是同一个场景；2）主体景物的形状、数量、位置必须与原片一致；3）画面被不规则水彩泼墨晕染边框包裹，颜料扩散斑驳水痕，半透明水彩质感，低饱和度柔和色彩，柔和平涂，纸张肌理，柔和自然光；4）具有明显的艺术笔触、颜料质感和纸张纹理，画面带不规则淡墨晕染毛边，颜料扩散斑驳水痕，半透明质感，同色系柔和低饱和，边缘留白。${textureText}${titleText}。这是明信片正面的插画部分，由程序自动排版，画面中绝对不能出现任何文字、字母、数字、标点、符号、标志或水印，绝对不能出现任何文字。`.replace(/\s+/g, ' ').trim();
}
