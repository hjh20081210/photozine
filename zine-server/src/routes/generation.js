  ...
  if (mode === 'POSTCARD') {
    if (isDouble) {
      ...
      const backNote = backMessage
        ...
        : '';
      const styleDraw = STYLE_PROMPTS[style] || STYLE_PROMPTS.watercolor;
      const frontWrap = hasPhoto
        ? `严格依据参考照片创作：在画面右侧约65%区域，忠实参考照片的构图、视角、主体、远近与色彩关系，将照片中的主景物（如森林木屋、帆船、雪山、风铃等）转化为克制的手绘插画（${styleDraw}），插画与原片主体高度相似、可辨认，只是转为插画质感；画面被不规则水彩淡墨晕染边框包裹，颜料扩散斑驳水痕，半透明水彩质感，低饱和柔和色彩，柔和平涂，边缘留白；左侧约35%区域为文字信息栏，垂直靠左对齐，顶部小数字配细分割横线，大写衬线标题，下方一行小写副标题，再下为 LOCATION/DATE 两行标签加空白填写横线，底部三个水彩晕染毛边圆角小色块`
      ...
      const frontWrap = hasPhoto
        ? `严格依据参考照片创作：在画面右侧约65%区域，忠实参考照片的构图、视角、主体、远近与色彩关系，将照片中的主景物转化为克制的手绘插画（${STYLE_PROMPTS[style] || STYLE_PROMPTS.watercolor}），插画与原片主体高度相似、可辨认，只是转为插画质感；画面被不规则水彩淡墨晕染边框包裹，颜料扩散斑驳水痕，半透明水彩质感，低饱和柔和色彩，柔和平涂；左侧约35%区域为文字信息栏，垂直靠左对齐，顶部小数字配细分割横线，大写衬线标题，下方一行小写副标题，再下为 LOCATION/DATE 两行标签加空白填写横线，底部三个水彩晕染毛边圆角小色块`
